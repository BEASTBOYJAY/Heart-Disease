...
page = st.sidebar.selectbox("Select Page", ["Exploratory Data Analysis","Explanation", "Model Training"])
...
if page == "Exploratory Data Analysis":
    ...
    trestbps=st.number_input("Resting Blood Pressure (trestbps)",0,None)
    chol=st.number_input("Cholestrol (chol)",0,None)
    fbs=st.number_input("Fasting Blood Pressure (fbs)",0,1)
    restecg=st.number_input("esting electrocardiographic (restecg)",0,1)
    thalach=st.number_input("maximum heart rate (thalach)",0,None)
    exang=st.number_input("exang",0,1)
    oldpeek=st.number_input("Oldpeek",0.0,None)
    slope=st.number_input("Slope",0,2)
    ca=st.number_input("ca",0,None)
    thal=st.number_input("thalium stress ",0,7)

    prd_data=pd.DataFrame(data=[[age,sex,cp,trestbps,chol,
                           fbs,restecg,thalach,exang,
                           oldpeek,slope,ca,thal]])
    ...
    for model_name in selected_models:
        ...
        if model_name == "Random Forest":
            ...
            if st.button("Random Forest", type='primary'):
                prediction = rf_rs.predict(prd_data)
                if prediction[0] == 1:
            ...
            # Predict if the person has heart disease or not
            if st.button("Logistic Regression", type='primary'):
                prediction = log_reg_rs.predict(prd_data)
                if prediction[0] == 1:
                    st.write("Prediction: The person has heart disease. 😷")
                else:
                    st.write("Prediction: The person does not have heart disease. 😁")
            ...
            # Predict if the person has heart disease or not
            if st.button("KNN", type='primary'):
                prediction = knn.predict(prd_data)
                if prediction[0] == 1:
                    st.write("Prediction: The person has heart disease. 😷")
                else:
                    st.write("Prediction: The person does not have heart disease. 😁")
    ...
    st.title("Explanation Of Upcoming Terms And Previous Data")
    st.subheader("Cross-Validation")
    st.write("Cross validation is a technique to evaluate the performance of a model on unseen data.")
    st.divider()
    st.subheader("Recall")
    st.write("Recall, also known as the true positive rate (TPR), is the percentage of data samples that a machine learning model correctly identifies as belonging to a class of interest—the “positive class”—out of the total samples for that class.")
    st.divider()
    st.subheader("Precision")
    st.write("the quality of a positive prediction made by the model")
    st.divider()
    st.subheader("Accuracy")
    st.write("Accuracy is a metric that measures how often a machine learning model correctly predicts the outcome.")
    st.divider()
    st.subheader("F1-Score")
    st.write("assesses the predictive ability of a model by examining its performance on each class individually rather than considering overall performance like accuracy does.")
    st.divider()
    st.subheader("Data Explaination")
    st.write("A. age - age in years.")
    st.write("B. sex - (1 = male; 0 = female).")
    st.write("C.cp - chest pain type.")
    ...
    st.write("D. trestbps - resting blood pressure (in mm Hg on admission to the hospital) anything above 130-140 is typically cause for concern.")
    st.write("chol - serum cholestoral in mg/dl.")
    st.write(" ➡ serum = LDL + HDL + .2 * triglycerides.")
    st.write(" ➡ above 200 is cause for concern.")
    st.write("fbs - (fasting blood sugar > 120 mg/dl) (1 = true; 0 = false).")
    st.write(" ➡ '>126' mg/dL signals diabetes.")
    st.write("E.restecg - resting electrocardiographic results.")
    st.write("F. thalach - maximum heart rate achieved.")
    st.write("G. exang - exercise induced angina (1 = yes; 0 = no).")
    st.write("H.oldpeak - ST depression induced by exercise relative to rest looks at stress of heart during excercise unhealthy heart will stress more.")
    st.write("I. oldpeak - ST depression induced by exercise relative to rest looks at stress of heart during excercise unhealthy heart will stress more.")
    st.write("J. slope - the slope of the peak exercise ST segment.")
    st.write("K. ca - number of major vessels (0-3) colored by flourosopy.")
    st.write("L. thal - thalium stress result.")
    st.write("M. target - have disease or not (1=yes, 0=no) (= the predicted attribute).")
